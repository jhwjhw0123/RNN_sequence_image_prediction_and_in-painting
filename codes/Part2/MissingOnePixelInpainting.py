...
import numpy as np
#Import the Data
OnePixelMissPackage = np.load('../CorruptData/one_pixel_inpainting.npy')
...
def OnePinxelInPainting(x):
    ...
    Imagewise_prediction = tf.reshape(predict_result,[n_chunks-1,2,1])  # n_chunck-1 * 2* 1

    #Pre-process test Data
    test_x = OnePixelMissInPainting.reshape([-1,n_chunks,chunck_size])
    test_x_list = np.split(test_x,ImageAmount,axis=0)           #ImageAmount * [n_chunks * chunk_size]
    true_x = OnePixelMissGroundTruth.reshape([-1,n_chunks,chunck_size])
    true_x_list = np.split(true_x,ImageAmount,axis=0)           #ImageAmount * [n_chunks * chunk_size]
    ...
    with tf.Session() as sess:
        ...
        for image in InpaintingList:
            ...
            #print(Missing_pixel_ind)
            #print(np.where(image == -1))
            Prediction_Length = ImageRange - Missing_pixel_ind + 1
            #print(InpaintingList[ImageIndex]
            input_pixel_x = np.reshape(test_x_list[ImageIndex],[1,n_chunks,chunck_size])
            input_pixel_x = np.repeat(input_pixel_x,2,axis=0)   # 2 * n_chunks * chunck_size
            #replace the correspoding pixel with all possible value
            for i in range(input_pixel_x.shape[0]):
                input_pixel_x[i][Missing_pixel_ind] = [i]      #[0] = 0; [1] = 1
            input_pixel_true_y = np.reshape(true_x_list[ImageIndex],[1,n_chunks,chunck_size])
            input_pixel_true_y = np.repeat(input_pixel_true_y,2,axis=0)   # 2 * n_chunks * chunck_size
            input_pixel_y = input_pixel_true_y.transpose([1,0,2])    # n_chunk * m_amount(2) * chunck_size
            first_pixel = np.reshape(input_pixel_y[1],[1,2,1])  # 1 * amounts(2) *1
            ...
            Predicted_Pixels = np.transpose(Imagewise_prediction.eval(MissingPixel_dict),[1,0,2])  # 2*[783 * 1]
            Predicted_Image = np.concatenate((first_pixel,Predicted_Pixels),axis=1)
            #print(np.shape(Imagewise_prediction.eval(MissingPixel_dict)))
            #print(np.shape(Predicted_Image_list))
            #Reconstruct the Image
            Predicted_Image_Collection.append(Predicted_Image)
            # *******************Probability and cross-entropy**************
            # Calculate the pixel probability
            Candidate_images_prob = prediction_eval.eval(MissingPixel_dict)   #n_chunck * 2(images) * 1
            Candidate_images_prob = np.transpose(np.reshape(Candidate_images_prob,[n_chunks-1,2,1]),[1,0,2])        #2 * n_chunck-1 * 1
            GroundTruthInput = np.reshape(GroundTruthList[ImageIndex],[1,n_chunks,chunck_size])       #1 * n_chuncks * 1
            GroundTruthY = GroundTruthInput.transpose([1,0,2])                                        #n_chunk * batch(1) * 1
            #print(np.shape(GroundTruthY))
            ...
            GroundTruthInput = np.repeat(GroundTruthInput,2,axis=0)                                   #2 * n_chuncks * 1
            ...
            Candidate_images_prob = np.reshape(Candidate_images_prob,[2,n_chunks-1])
            ...
            Fill_in_digit_Collection.append(Best_fill_index)
        ...
        np.save('../Plot/Data/Question3_OneMising_LSTM_Heiarchy/InPainting_Cross_entropy_Collection',np.asarray(InPainting_Cross_entropy_Collection))
        np.save('../Plot/Data/Question3_OneMising_LSTM_Heiarchy/GroundTruth_Cross_entropy_Collection',np.asarray(GroundTruth_Cross_entropy_Collection))
        np.save('../Plot/Data/Question3_OneMising_LSTM_Heiarchy/BestFillIndex',np.asarray(Fill_in_digit_Collection))
        np.save('../Plot/Data/Question3_OneMising_LSTM_Heiarchy/PredictedImages',np.asarray(Predicted_Image_Collection))

#Implement above functions
OnePinxelInPainting(x)